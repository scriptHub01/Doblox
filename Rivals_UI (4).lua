-- Rivals Ragebot v4 (HRK style desync + proper wallbang)
-- Delta optimized
...
local Settings = {
    ...
    Wallbang = true,
    Desync = true,           -- 머리 위 텔포 방식
    DesyncHeight = 5.5,      -- 머리 위 높이
...
pcall(function()
    ...
    if typeof(oldGetConnections) == "function" then
        getconnections = function(signal)
            local cons = oldGetConnections(signal)
            for i = #cons, 1, -1 do
                local ok, fn = pcall(function() return cons[i].Function end)
                if ok and fn then
                    local info = debug.getinfo(fn)
                    if info and info.source then
                        local src = tostring(info.source):lower()
                        if src:find("anti") or src:find("detect") or src:find("ban") or src:find("kick") then
...
pcall(function()
    ...
    oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if method == "FireServer" or method == "InvokeServer" then
            ...
            or n:find("flag") or n:find("report") or n:find("security") then
                return
            end
        end
            ...
            if typeof(args[1]) == "Vector3" and typeof(args[2]) == "Vector3" then
                ...
                if typeof(params) == "RaycastParams" then
                    ...
                    if workspace:FindFirstChild("Map") then
                        table.insert(params.FilterDescendantsInstances, workspace.Map)
                    end
                    if workspace:FindFirstChild("Terrain") then
                        table.insert(params.FilterDescendantsInstances, workspace.Terrain)
                    end
                end
            end
        end
...
Main.Size = UDim2.new(0, 410, 0, 380)
Main.Position = UDim2.new(0.5, -205, 0.5, -190)
...
CreateToggle(CombatPage, "Desync (머리위 텔포)", Settings.Desync, function(v) Settings.Desync = v end, 28)
CreateToggle(CombatPage, "Silent Aim", Settings.SilentAim, function(v) Settings.SilentAim = v end, 56)
CreateToggle(CombatPage, "Auto Fire (FOV)", Settings.AutoFire, function(v) Settings.AutoFire = v end, 84)
...
info.Text = "Insert = UI 토글\n\nRagebot + Desync = 상대 머리 위로\n잠깐 텔포 → 조준 → 발사 → 복구\n\nAutoFire는 FOV 안에 있을 때만"
...
RunService.Heartbeat:Connect(function()
    ...
    -- Combat (HRK desync 방식)
    if not Settings.Ragebot then return end
    if not myRoot then return end

    local targetHead, screenDist = GetClosestEnemy()
    if not targetHead then return end

    -- Desync: 상대 머리 위로 잠깐 텔포 → 조준 → 복구
    if Settings.Desync then
        ...
        local desyncPos = targetHead.Position + Vector3.new(0, Settings.DesyncHeight, 0)
        ...
        -- 다음 프레임에 복구
        RunService:BindToRenderStep("RivalsRestore", 200, function()
            if myRoot then
                myRoot.CFrame = oldCF
                myRoot.AssemblyLinearVelocity = oldVel
                myRoot.AssemblyAngularVelocity = oldRot
            end
            RunService:UnbindFromRenderStep("RivalsRestore")
        end)
    end

    -- Silent Aim (머리로)
    if Settings.SilentAim then
        local sp, onScreen = Camera:WorldToViewportPoint(targetHead.Position)
        if onScreen then
        ...
        -- 카메라도 머리 방향으로
        pcall(function()
            Camera.CFrame = Camera.CFrame:Lerp(CFrame.lookAt(Camera.CFrame.Position, targetHead.Position), 0.35)
        end)
    end
    ...
    if Settings.AutoFire and screenDist and screenDist <= Settings.FOV then
        local now = tick()
        local delay = Settings.RapidFire and Settings.FireDelay or 0.08
...
print("[Rivals] Rage v4 | Insert = UI | Desync + Wallbang ready")
